  ...
  <section className="py-24 md:py-32 bg-muted/40">
    <div className="container px-6 max-w-5xl">
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5 mb-20">
        {benefits.map((b, i) => (
          <motion.div
            ...
            whileHover={{ y: -4, boxShadow: "0 16px 32px -8px rgba(0,0,0,0.08)", transition: { duration: 0.25 } }}
            ...
            <tbody>
              {comparison.map(([concept, trad, sysde], i) => (
                ...
                >
                  ...
                  <td className="py-3 px-5 text-center font-bold text-sysde-blue">{sysde}</td>
                </motion.tr>
              ))}
            </tbody>
          </table>
      ...
      <motion.div
        ...
        className="mt-20"
      >
        <h3 className="text-xl font-bold text-foreground mb-1 text-center">Cronograma de Implementación</h3>
        <p className="text-sm text-muted-foreground text-center mb-6">Metodología SYSDE "Agile - Answers" · 6 meses</p>
        <div className="overflow-x-auto rounded-2xl border border-border bg-card">
          <table className="w-full min-w-[700px] text-sm">
            <thead>
              <tr className="border-b border-border">
                <th className="text-left py-3 px-4 font-semibold text-muted-foreground w-12">Etapa</th>
                <th className="text-left py-3 px-4 font-semibold text-muted-foreground">Tareas</th>
                {[1,2,3,4,5,6].map(m => (
                  <th key={m} className="text-center py-3 px-2 font-bold text-sysde-red w-12">M{m}</th>
                ))}
              </tr>
            </thead>
            <tbody>
                    ...
                    <td key={mi} className="py-3 px-2 text-center">
                      {active ? (
                        <motion.div
                          ...
                          className={`w-6 h-2 rounded-full mx-auto origin-left ${i % 2 === 0 ? 'bg-sysde-red' : 'bg-sysde-blue'}`}
                        />
                      ) : null}
                    </td>
                  ))}
